...
export class UtilisateursComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.liststructurefiltre=[]
        this.liststructurefiltre.push({id:0,libelle:"toutes les Structures"})
        ...
        this.http.get(environment.apiUrl + '/dg_Structure').subscribe(
            (data)=>
            {
                    this.Allstructure=data
                    this.tempAllstructure=data

console.log(this.userConnecte.dg_structure.dg_drp.id)

                    if (this.findRole(['ROLE_DRP'])==true)
                    {
                        this.Allstructure=this.Allstructure.filter(structure=>structure.dg_drp.id==this.userConnecte.dg_structure.dg_drp.id && structure.id!=this.userConnecte.dg_structure.id)
                        this.tempAllstructure==this.Allstructure
                    }
            }
        )
        }
    ...
    filterStructure(event) {
        ...
        const filtered: Structure[] = [];
                ...
                for (let i = 0; i < this.Allstructure.length; i++) {
                    const struc = this.Allstructure[i];
                    if (struc.libelle.toLowerCase().indexOf(query.toLowerCase()) == 0) {
                        filtered.push(struc);
                    }
                }
    
                this.liststructurefiltre=this.liststructurefiltre.concat(filtered);
      ...
      SelectStructure()
      {
        ...
        this.user=this.usertemp
          ...
          if(this.structureChoisi.id!=0)
          {
              this.user=this.user.filter(utilisateur=>utilisateur.dg_structure.id==this.structureChoisi.id)
          }
      }
    ...
    filtre(){
        if(this.findRole(['ROLE_DRP'])==true){
          /*  this.usertemp = this.usertemp.filter(use => use.dg_structure.id === this.utilisateur.dg_structure.id);
            this.user=this.usertemp;
            */
            this.usertemp = this.usertemp.filter(use => use.dg_structure.dg_drp.id === this.utilisateur.dg_structure.dg_drp.id && use.dg_structure.id != this.utilisateur.dg_structure.id);
            console.log(this.utilisateur.dg_structure.dg_drp.id)
            console.log(this.usertemp)
            this.user=this.usertemp;
        }
        else if(this.findRole(['ROLE_RECEVEUR'])==true) {
            this.usertemp = this.usertemp.filter(use => use.dg_structure.id === this.utilisateur.dg_structure.id && use.id!=this.utilisateur.id);
            this.user=this.usertemp;
            this.structurs=[]
...
activate()
{
    ...
    filtrebis(){
        if(this.findRole(['ROLE_DRP'])==true){
            this.directioon = this.directioon.filter(use => use.dg_drp.id === this.utilisateur.dg_structure.dg_drp.id && use.id !=this.userConnecte.dg_structure.id);
            console.log(this.directioon)
        }
        else if(this.findRole(['ROLE_DER'])==true){
            this.directioon = this.directioon.filter(use => use.dg_typeStructure.libelle === "DIRECTION");
            console.log(this.directioon)
        }
    }
    createId(): string {
