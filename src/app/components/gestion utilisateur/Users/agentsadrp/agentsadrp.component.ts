...
import { Sauvegarde } from '../agent-annexe/Sauvegarde';

@Component({
  selector: 'app-agentsadrp',
  templateUrl: './agentsadrp.component.html',
  styleUrls: ['./agentsadrp.component.scss']
})
export class AgentsadrpComponent implements OnInit {
  ...
  saveUser() {
      ...
      if (this.utilisateur.nom.trim() && this.utilisateur.prenom.trim() && this.utilisateur.email.trim() && this.utilisateur.matricule.trim()) {
          if (this.utilisateur.id) {
              ...
              this.http.post<any>(environment.apiUrl+'/dg_User', this.utilisateur).subscribe((data) => {
