    ...
    template:`
        <div class="users-content">
                    <div class="card">
                        ...
                        <p-table #dt [value]="user" responsiveLayout="scroll" [rows]="10" [globalFilterFields]="['nom','prenom','email']" [rows]="10" [paginator]="true" [rowsPerPageOptions]="[10,20,30]" [showCurrentPageReport]="true" currentPageReportTemplate="Affichage des {first} aux {last}èmes des  {totalRecords} entrées" [(selection)]="selectedUsers" selectionMode="multiple" [rowHover]="true" dataKey="id">
...
export class Users_annexeComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if(this.sauvegarde.Users_anx_Information && this.sauvegarde.Structure_anx_Information){
        ...
        this.organisationService.getAllUsers().subscribe(
            (data) => {
                this.user = data;
                    if(this.sauvegarde.Structure_anx_Information) {
                        this.user = this.user.filter(use => use.dg_structure.id === this.sauvegarde.Structure_anx_Information.id);
                        this.attente=true;
                    }
            }
        );
        ...
        if(this.sauvegarde.Structure_anx_Information){
            this.titre=this.sauvegarde.Structure_anx_Information.libelle
        }
    ...
    filterDirectionsbis(event) {
        ...
        this.direction = this.direction.filter(use => use.id === this.sauvegarde.Structure_anx_Information.id);
    ...
    filterDirections(event) {
        const filtered: Structure[] = [];
        const query = event.query;
        this.directioon = this.directioon.filter(use => use.dg_drp.id === this.sauvegarde.Users_anx_Information.dg_structure.dg_drp.id && (use.dg_typeStructure.libelle === "Bureau" || use.dg_typeStructure.libelle === "Annexe"));
    ...
    filtre(){
        if(this.findRole(['ROLE_DER', 'ROLE_DRP', 'ROLE_RECEVEUR'])==true){
            this.annexes = this.annexes.filter(use => use.bureau.id === this.sauvegarde.Structure_anx_Information.id);
