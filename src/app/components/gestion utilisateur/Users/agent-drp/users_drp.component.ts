    ...
    template:`
        <div class="users-content">
                    <div class="card">
                        ...
                        <p-dialog [(visible)]="productDialog" [style]="{width: '700px'}" header="{{entete}}" [modal]="true" class="p-fluid">
                            ...
                            <ng-template pTemplate="footer">
                                ...
                                <button pButton pRipple label="Savee" icon="pi pi-check" class="p-button-text" (click)="Verif()"></button>
                            </ng-template>
                        </p-dialog>
                        ...
                        <p-dialog [(visible)]="structDialog" [style]="{width: '700px', height:'400px'}" header="Transfert {{nom}}" [modal]="true" class="p-fluid">
                            ...
                            <ng-template pTemplate="footer">
                                <button pButton pRipple label="Cancel" icon="pi pi-times" class="p-button-text"
                                        (click)="hideDialog()"></button>
                                <button pButton pRipple label="Save" icon="pi pi-check" class="p-button-text" (click)="saveUserprime()"></button>
                            </ng-template>
                        </p-dialog>
...
Pii 5D 3V
 */
export class Users_drpComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if(this.sauvegarde.Users_drp_Information && this.sauvegarde.Structure_drp_Information){

        }
        else{
            this.router.navigate(['/Users_bureaux/Structures']);
        }
        ...
        this.organisationService.getAllUsers().subscribe(
            (data) => {
                this.user = data;
                    if(this.sauvegarde.Structure_drp_Information) {
                        this.user = this.user.filter(use => use.dg_structure.id === this.sauvegarde.Structure_drp_Information.id);
                    }
                    this.attente=true;
            }
        );
        this.organisationService.getAllDirections().subscribe(
            (data)=>{
                ...
                this.last=this.directioon;
                this.last.sort((a,b) => a.libelle.localeCompare(b.libelle));
            }
        );
        ...
        if(this.sauvegarde.Structure_drp_Information){
            this.titre=this.sauvegarde.Structure_drp_Information.libelle
        }
    ...
    openNew() {
        ...
        this.entete="Creation utilisateur du "+this.sauvegarde.Structure_drp_Information.libelle;
    }
    openNew2() {
        this.utilisateur = {};
        this.submitted = false;
        this.productDialog = true;
        this.annex = true;
        this.entete="Creation utilisateur de l'"+this.lastStr.libelle;
    }
    ...
    saveUser() {
        ...
        console.log(this.utilisateur)
        if(this.annex){
            this.utilisateur.dg_structure=this.lastStr;
        }
        else{
            this.utilisateur.dg_structure=this.sauvegarde.Structure_drp_Information;
        }
    ...
    filterDirectionsbis(event) {
        ...
        this.direction = this.direction.filter(use => use.id === this.sauvegarde.Structure_drp_Information.id);
    ...
    filterDirections(event) {
        const filtered: Structure[] = [];
        const query = event.query;
        this.directioon = this.directioon.filter(use => use.dg_drp.id === this.sauvegarde.Users_drp_Information.dg_structure.dg_drp.id && (use.dg_typeStructure.libelle === "Bureau" || use.dg_typeStructure.libelle === "Annexe"));
    ...
    filterFonctions(event) {
        ...
        const query = event.query;
     //   this.fonction = this.fonction.filter(use => use.id == 3 || use.id == 6 || use.id == 10 || use.id == 11 || use.id == 8 || use.id == 9 || use.id == 7 || use.id == 4);
    ...
    filtre(){
        if(this.findRole(['ROLE_DER', 'ROLE_DRP', 'ROLE_RECEVEUR'])==true){
            this.annexes = this.annexes.filter(use => use.bureau.id === this.sauvegarde.Structure_drp_Information.id);
    ...
    showDialog(annexe) {
        ...
        this.organisationService.getAllUsers().subscribe(
            (data) => {
                ...
                this.last = this.last.filter(use => use.id === this.annexe.annexe.id);
                this.lastStr=this.last[0];
                this.attenteb=true;
            }
        );
    }
