...
@Component({
  selector: 'app-agent-receveur',
  templateUrl: './agent-receveur.component.html',
  styleUrls: ['./agent-receveur.component.scss']
})
export class AgentReceveurComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.organisationService.getAllUsers().subscribe(
            (data) => {
                this.user = data;
                if(this.sauvegarde.Structure_rcv_Information) {
                    this.user = this.user.filter(use => use.dg_structure.id === this.sauvegarde.Structure_rcv_Information.id);
                }
            }
        );
    ...
    filterDirectionsbis(event) {
        ...
        this.direction = this.direction.filter(use => use.id === this.sauvegarde.Structure_rcv_Information.id);
    ...
    filterDirections(event) {
        const filtered: Structure[] = [];
        const query = event.query;
        this.directioon = this.directioon.filter(use => use.dg_drp.id === this.sauvegarde.Users_rcv_Information.dg_structure.dg_drp.id && (use.dg_typeStructure.libelle === "Bureau" || use.dg_typeStructure.libelle === "Annexe"));
    ...
    filtre(){
        if(this.findRole(['ROLE_DER', 'ROLE_DRP', 'ROLE_RECEVEUR'])==true){
            this.annexes = this.annexes.filter(use => use.bureau.id === this.sauvegarde.Structure_rcv_Information.id);
